...
def load_data_since_case_fifty(drop_states=False, p_crit=.045, filter_n_days_10=None):
    ...
    for country in df.country.unique():
        if not df.loc[(df.country == country), 'state'].isnull().all():
            for state in df.loc[(df.country == country), 'state'].unique():
                df.loc[(df.country == country) & (df.state == state), 'days_since_100'] = \
                    np.arange(-len(df.loc[(df.country == country) & (df.state == state) & (df.confirmed < 10)]), 
                              len(df.loc[(df.country == country) & (df.state == state) & (df.confirmed >= 10)]))
        else:
            df.loc[(df.country == country), 'days_since_10'] = \
                np.arange(-len(df.loc[(df.country == country) & (df.confirmed < 10)]), 
                          len(df.loc[(df.country == country) & (df.confirmed >= 10)]))
    ...
    if filter_n_days_10 is not None:
        # Select countries for which we have at least some information
        countries = pd.Series(df.loc[df.days_since_10 >= filter_n_days_10].country.unique())
        df = df.loc[lambda x: x.country.isin(countries)]
